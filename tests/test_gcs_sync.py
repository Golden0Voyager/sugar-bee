...
def gcs_env(monkeypatch):
    ...
    monkeypatch.setattr(gcs_sync, 'DB_NAME', DB_NAME)
...
class TestGetGcsBucket:
    def test_no_bucket_configured(self, monkeypatch):
        monkeypatch.setattr(gcs_sync, 'GCS_BUCKET_NAME', '')
        assert gcs_sync.get_gcs_bucket() is None
    ...
    def test_returns_bucket(self, gcs_env, mock_storage):
        ...
        result = gcs_sync.get_gcs_bucket()
        client.bucket.assert_called_once_with('test-bucket')
        assert result is bucket


class TestRestoreDbFromGcs:
    def test_no_bucket_skips(self, monkeypatch):
        monkeypatch.setattr(gcs_sync, 'GCS_BUCKET_NAME', '')
        gcs_sync.restore_db_from_gcs()
    ...
    def test_downloads_latest_blob(self, gcs_env, mock_storage, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gcs_sync, 'DB_NAME', str(db_path))
        blobs = [
...
class TestSyncFile:
    def test_sync_to_gcs_missing_local(self, gcs_env, mock_storage, tmp_path):
        ...
        gcs_sync.sync_file_to_gcs(str(missing), 'avatars/avatar.png')
        bucket.blob.assert_not_called()
    ...
    def test_sync_to_gcs_uploads(self, gcs_env, mock_storage, tmp_path):
        ...
        gcs_sync.sync_file_to_gcs(str(local), 'avatars/avatar.png')
    ...
    def test_sync_from_gcs_blob_missing(self, gcs_env, mock_storage, tmp_path):
        ...
        gcs_sync.sync_file_from_gcs('garmin_tokens/garmin_tokens.json', str(local))
    ...
    def test_sync_from_gcs_no_bucket(self, monkeypatch, tmp_path):
        monkeypatch.setattr(gcs_sync, 'GCS_BUCKET_NAME', '')
        gcs_sync.sync_file_from_gcs('garmin_tokens/garmin_tokens.json', str(tmp_path / 'x.json'))
