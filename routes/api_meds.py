...
def get_medication_plan(plan_id):
    try:
        ...
        c = db.cursor()
        current_user_id = user_manager.get_current_user_id()
        c.execute("SELECT * FROM medication_plans WHERE id = ? AND user_id = ?", (plan_id, current_user_id))
        row = c.fetchone()
        if row:
...
def update_medication_plan(plan_id):
    try:
        ...
        c.execute("SELECT dosage, dose_quantity, dose_unit FROM medication_plans WHERE id = ? AND user_id = ?", (plan_id, current_user_id))
        old = c.fetchone()
        if old:
        ...
        c.execute("""UPDATE medication_plans SET
                 ...
                 (data.get('medication_name'), data.get('dosage'), data.get('times_per_day', 1),
                  ...
                  data.get('med_type', ''), plan_id, current_user_id))
...
def delete_medication_plan(plan_id):
    try:
        ...
        c = db.cursor()
        current_user_id = user_manager.get_current_user_id()
        c.execute("DELETE FROM medication_logs WHERE plan_id = ? AND plan_id IN (SELECT id FROM medication_plans WHERE user_id = ?)", (plan_id, current_user_id))
        c.execute("DELETE FROM dosage_history WHERE plan_id = ? AND plan_id IN (SELECT id FROM medication_plans WHERE user_id = ?)", (plan_id, current_user_id))
        c.execute("DELETE FROM medication_plans WHERE id = ? AND user_id = ?", (plan_id, current_user_id))
        db.commit()
...
def toggle_medication_plan(plan_id):
    try:
        ...
        c = db.cursor()
        current_user_id = user_manager.get_current_user_id()
        c.execute("UPDATE medication_plans SET is_active = NOT is_active WHERE id = ? AND user_id = ?", (plan_id, current_user_id))
        if c.rowcount == 0:
            return jsonify({"error": "Plan not found"}), 404
        db.commit()
