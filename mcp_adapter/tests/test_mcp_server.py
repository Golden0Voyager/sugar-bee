"""
mcp_adapter/server.py 单元测试 (0% → ~85%)
...
class TestNormalizeTimestamp:
    ...
    def test_none_returns_now(self):
        from mcp_adapter.server import _normalize_timestamp
...
class TestApiHeaders:
    def test_returns_correct_headers(self):
        from mcp_adapter.server import _api_headers
        with patch('mcp_adapter.server.AGENT_API_TOKEN', 'test-token'):
...
class TestIsDupError:
    def test_dup_error_returns_message(self):
        from mcp_adapter.server import _is_dup_error
...
class TestValidateRecordData:
    ...
    def test_valid_no_warnings(self):
        from mcp_adapter.server import _validate_record_data
        r = {"value": 5.5}
...
class TestBpStatus:
    def test_high(self):
        from mcp_adapter.server import _bp_status
...
class TestValidateBp:
    def test_valid(self):
        from mcp_adapter.server import _validate_bp
...
class TestValidateGlucose:
    def test_valid(self):
        from mcp_adapter.server import _validate_glucose
...
class TestValidateWeight:
    def test_valid(self):
        from mcp_adapter.server import _validate_weight
...
class TestValidateHeartRate:
    def test_valid(self):
        from mcp_adapter.server import _validate_heart_rate
...
class TestUserLabel:
    def test_returns_label(self, patch_db):
        from mcp_adapter.server import _user_label
...
class TestGetUserInfo:
    def test_user_not_found(self, patch_db):
        from mcp_adapter.server import get_user_info
...
class TestBatchRecord:
    ...
    async def test_db_exception_rollback(self, patch_db):
        ...
        with patch('mcp_adapter.server._db', return_value=bad_conn):
            result = await batch_record(1, records=[
                {"type": "空腹", "value": 5.5, "datetime": "2026-05-02 07:00"}
            ])
        assert "❌" in result
...
class TestBatchParseAndRecord:
    ...
    async def test_regex_fast_path(self, patch_db):
        ...
        with patch('mcp_adapter.server._normalize_timestamp', return_value=ts), \
