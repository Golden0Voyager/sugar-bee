...
def build_pdf(output_path):
    ...
    t = make_table(["时间段", "整体表现", "血糖数值", "原因分析"],
                    stage_data, [width*0.22, width*0.17, width*0.30, width*0.31], st)
    story.append(t)
    ...
    t = make_table(["药物名称", "您的用量", "通俗原理", "用量说明"],
                    med_data, [width*0.22, width*0.17, width*0.30, width*0.31], st)
    story.append(t)
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph(
    ...
    t = make_table(["药物", "调整方案", "要点", "备注"],
                    summary_data, [width*0.22, width*0.28, width*0.25, width*0.25], st)
